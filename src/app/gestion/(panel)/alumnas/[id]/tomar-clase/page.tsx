...
import { nextDateForWeekday, todayYMD, WEEKDAY_LABELS, weekdayOfYMD } from "@/lib/dates";
        ...
        <Card>
          <TakeClassForm
            ...
            activities={sorted.map((a) => {
              // El saldo se calcula para la PRÓXIMA fecha real del horario
              // (no siempre "hoy"): un pack puede vencer o empezar entre medio.
              const fecha = nextDateForWeekday(today, a.weekday);
              return {
                id: a.id,
                label: `${a.discipline.name} · ${WEEKDAY_LABELS[a.weekday]} ${a.startTime}`,
                weekday: a.weekday,
                autoDateYMD: fecha,
                balance: availableBalance(packs, a.disciplineId, fecha),
              };
            })}
          />
        </Card>
